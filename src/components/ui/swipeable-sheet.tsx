  ...
  "fixed z-50 gap-4 bg-background p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
        ...
        bottom:
          "inset-x-0 bottom-0 border-t data-[state=closed]:slide-out-to-bottom data-[state=open]:slide-in-from-bottom max-h-[85vh] overflow-y-auto",
  ...
  (
    ...
    const handleTouchStart = (e: React.TouchEvent) => {
      if (side === "bottom" || side === "top") {
        ...
        // Permitir scroll normal dentro do conteúdo
        if (contentElement && target !== contentElement) {
          const scrollableParent = target.closest('[data-scrollable]');
          if (scrollableParent) {
            return; // Não interferir com o scroll
          }
        }
